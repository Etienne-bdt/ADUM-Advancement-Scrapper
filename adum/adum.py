...
class adum(requests.Session):
    ...
    def get_formation_info(self, formation_url: str):
        ...
        for b_tag in formation_section.find_all_next("b"):
            session_info = {}
    ...
    def _parse_session_info(self, session) -> dict:
        ...
        for strings in session:
            if ":" in strings:
                ...
                # Additional details without a key
                if "Info diverses" not in info:
                    info["Info diverses"] = ""
                info["Info diverses"] = strings.strip()
        return info
    ...
    def _convert_session_to_ical(self, session) -> icalendar.Event:
        ...
        info = self._parse_session_info(session)
        ...
        if 'Date' in info:
            ...
            try:
                ...
                startend_time = re.findall(r'(\d{1,2}h\s?\d{2})', info.get('Horaires', ''))
                if len(startend_time) == 2:
                    start_time_str, end_time_str = startend_time
                    start_time = datetime.datetime.strptime(start_time_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    end_time = datetime.datetime.strptime(end_time_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    event.add('dtstart', datetime.datetime.combine(start_date.date(), start_time))
                    event.add('dtend', datetime.datetime.combine(start_date.date(), end_time))
                elif len(startend_time) == 4:
                    start_time_str, end_time_str = startend_time[:2]
                    start_time2_str, end_time2_str = startend_time[2:]
                    start_time = datetime.datetime.strptime(start_time_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    end_time = datetime.datetime.strptime(end_time_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    start_time2 = datetime.datetime.strptime(start_time2_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    end_time2 = datetime.datetime.strptime(end_time2_str.replace('h', ':').replace(' ', ''), "%H:%M").time()
                    # Create two events
                    event.add('summary', session['title'])
                    event.add('dtstart', datetime.datetime.combine(start_date.date(), start_time))
                    event.add('dtend', datetime.datetime.combine(start_date.date(), end_time))
                    ...
                    event2.add('dtstart', datetime.datetime.combine(start_date.date(), start_time2))
                    event2.add('dtend', datetime.datetime.combine(start_date.date(), end_time2))
                    return [event, event2]
                else:
        ...
        description = "\n".join(f"{k}: {v}" for k, v in info.items() if k != 'Date')
